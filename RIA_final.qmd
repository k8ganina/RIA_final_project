...
## 4.6. Addressing potential endogeneity of RTAs (to correct for reverse causality)
...
## Overview
```{r}
#note: here RVRS is reverse causality regression from section 5
...
table_asean_detailed <- huxreg("Directional" = rta_poisson_intra_dir,
                    ...
                    number_format = 3,
                    error_pos = "same",
                    statistics = c("N"="nobs", "pseudo R^2"="pseudo.r.squared"),
                    note = "Notes: Statistics based on author's calculations. All estimates are obtained with data for the period from 1965 to 2020 with 3-year intervals. Standard errors are clustered by country pair and are reported in parentheses. {stars}") %>%
  insert_row("","(1) PPML", "(2) PPML", after = 0) %>%
  insert_row(c("Fixed Effects", "Yes", "Yes"), after = 21) %>%
  insert_row(c("Intra-national trade", "Yes", "Yes"), after = 22) %>%
  #insert_row(c("Japan in other RTAs", "No", "No", "Yes", "No", "Yes", "No", "Yes"), after = 38) %>%
  set_col_width(c(0.3,rep(0.9/2,2))) %>%
  set_align(everywhere,-1,"center") %>%
  set_top_border(1,everywhere,1) %>%
...
```{r}
#Estimation
# rta_glob_asean = fepois(tradeflow_comtrade_d ~ gatt_wto_both + rta_excl_asean_plus + rta_asean_plus + rta_asean_plus_lag3 + rta_asean_plus_lag6 + rta_asean_plus_lag9 + rta_excl_asean_plus + rta_excl_asean_plus_lag3 + rta_excl_asean_plus_lag6 + rta_excl_asean_plus_lag9 +
#                    intl_brdr_1986 + intl_brdr_1989 + intl_brdr_1992 + intl_brdr_1995 + intl_brdr_1998 + intl_brdr_2001 + intl_brdr_2004 + intl_brdr_2007 + intl_brdr_2010 + intl_brdr_2013 + intl_brdr_2016 + intl_brdr_2019 |
#                     fe_exp_year + fe_imp_year + pair_id,
#                   data = asean_selected_rta, 
#                   vcov = cluster ~ pair_id)
# 
# summary(rta_glob_asean)

##doesn't work as it should be, results are super weird
```
