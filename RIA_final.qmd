...
## Estimation 
fit_fixedeffects = feols(ln_tradeflow_comtrade_d ~ ln_dist + comlang_off + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig |
...
```{r}
## PPML model
fit_poisson = fepois(tradeflow_comtrade_d ~ ln_dist + comlang_off + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig |
                           fe_exp_year + fe_imp_year,
...
# Estimates of RTAs (general RTA, not ASEAN specific)
...
## 1. Traditional estimates of RTAs (OLS)

```{r}
## OLS estimation
rta_ols = feols(ln_tradeflow_comtrade_d ~ ln_dist + comlang_off + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig + rta |
                           fe_exp_year + fe_imp_year,
...
## 2. Traditional estimates of RTAs (PPML)
```{r}
rta_poisson = fepois(tradeflow_comtrade_d ~ ln_dist + comlang_off + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig + rta |
                           fe_exp_year + fe_imp_year,
data = asean_selected_rta %>%
...
rta_poisson_types = fepois(tradeflow_comtrade_d ~ ln_dist + comlang_off + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig + as.factor(rta_type) |
                           fe_exp_year + fe_imp_year,
data = asean_selected_rta %>%
filter(iso3_o != iso3_d),
vcov = cluster ~ pair_id)
...
## 3. Allowing for trade diversion from domestic sales (includes FE for intra-trade)
```{r}
## PPML estimation
rta_poisson_intra = fepois(tradeflow_comtrade_d ~ ln_dist + comlang_off + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig + rta |
...
## 4. Addressing potential endogeneity of RTAs
```{r}
## PPML estimation
...
# Estimates of RTAs (ASEAN specific)
...
asean_selected_rta <- asean_selected_rta %>%
  mutate(rta_excl_asean_plus = if_else(rta_asean_plus==1, 0, as.numeric(rta)),
         rta_type_excl_asean_plus = if_else(rta_asean_plus==1, 0, as.numeric(rta_type)))
...
# RTA Robustness Checks (not ASEAN specific)

## 1. Testing for potential reverse causality between trade and RTAs
```{r}
...
## 3. Addressing globalization effects
