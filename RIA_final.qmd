...
## 1. Traditional estimates of RTAs
```{r}
...
## OLS estimation
rta_ols = feols(ln_tradeflow_comtrade_d ~ ln_dist + comlang_off + comlang_ethno + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig + col_dep_end_conflict + rta |
                           fe_exp_year + fe_imp_year,
data = asean_selected_tradit %>%
...
summary(rta_ols)
    ...
    #GAELLE: we have again R²=1, maybe we should check that again

## PPML estimation
rta_poisson = fepois(tradeflow_comtrade_d ~ ln_dist + comlang_off + comlang_ethno + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig + col_dep_end_conflict + rta |
                           fe_exp_year + fe_imp_year,
data = asean_selected_tradit %>%
...
summary(rta_poisson)
    ...
    #GAELLE: better R² but weird value for RTAs that seem to destroy trade -> maybe check robustness
```

## 2. Allowing for trade diversion from domestic sales
```{r}
## PPML estimation
rta_poisson_intra = fepois(tradeflow_comtrade_d ~ ln_dist + comlang_off + comlang_ethno + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig + col_dep_end_conflict + rta |
...
```{r}
## PPML estimation
rta_endo = fepois(tradeflow_comtrade_d ~ rta |
fe_exp_year + fe_imp_year + pair_id,
data = asean_selected_tradit,
vcov = cluster ~ pair_id)

## Head of results
head(asean_selected_tradit[-rta_endo$obs_selection$obsRemoved,
...
## 4. Testing for potential reverse causality between trade and RTAs
```{r}
...
#GAELLE: Idk why but there is a problem in the formula here
```
