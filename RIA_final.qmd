...
# ADELE: for this one I decided to add another filter at the beginning to get only the trade flows>0 with the code: 
## Select necessary years with 4 year intervals: asean_selected_tradit <- data_asean_selected %>% filter(year %in% seq(1965, 2021, 4)& tradeflow_comtrade_d>0)
#By doing this, I can get the log for the variables (otherwise I couldn't) and I can avoid the -inf. I know that the teacher did it also during running the regression but for me it was better to specify it before otherwise I couldn't run his code. 
```
...
fit_ols2 <- feols(ln_tradeflow_comtrade_d ~ ln_dist + ln_gdp_o + ln_gdp_d + comlang_off + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig,
                ...
                vcov = cluster ~ pair_id)
summary(fit_ols2) 
#ADELE: For me results are less significants with true gdp values x-10 (??) + better R^2 with using the proxies for GDP (62%)
```
...
##Estimation
fit_remoteness = feols(ln_tradeflow_comtrade_d ~ ln_dist + comlang_off + comlang_ethno + comcol + col45 + comleg_pretrans + comleg_posttrans + comrelig + col_dep_end_conflict + ln_Y_it + ln_E_jt + ln_remoteness_exp + ln_remoteness_imp,
                       data = asean_selected_tradit %>% filter(tradeflow_comtrade_d > 0 & iso3_o != iso3_d),
                       vcov = cluster ~ pair_id)
summary(fit_remoteness)# better R^2 (65%) with adding remoteness
```
