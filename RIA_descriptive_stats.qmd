...
ggplot(total_trade_ajcep, aes(x = year)) +
...
```{r}
# We firstly focus on Japan's total exports to each ASEAN countries in 2000 and 2012.
...
```{r}
# We can now focus on Japan's total imports from each ASEAN countries in 2000 and 2012.
...
```{r}
# Imports of Japan from all the world.

imports_japan_world <- subset(trade_japan_world, iso3_d=="JPN") %>%
  filter(year %in% c(2000, 2012) & iso3_o != "JPN") %>%
  group_by(year, iso3_o)

# Top 5 imports partners of Japan in 2000

top_imports_2000_sum <- imports_japan_world %>%
  filter(year == 2000) %>%
  group_by(iso3_o) %>%
  summarize(total_imports = sum(tradeflow_comtrade_d)) %>%
  top_n(5, total_imports) %>%
  arrange(desc(total_imports))

# Plot the graph for 2000

ggplot(top_imports_2000_sum, aes(x = "", y = total_imports, fill = iso3_o)) + 
...
# Top 5 import partners of Japan in 2012
...
# Plot the graph for 2012

ggplot(top_imports_2012_sum, aes(x = "", y = total_imports, fill = iso3_o)) + 
  geom_bar(width = 1, stat = "identity") +
  coord_polar(theta = "y") +
...
# Top 5 exports partners of Japan in 2000

top_exports_2000_sum <- exports_japan_world %>%
  filter(year == 2000) %>%
  group_by(iso3_d) %>%
  summarize(total_exports = sum(tradeflow_comtrade_d)) %>%
  top_n(5, total_exports) %>%
  arrange(desc(total_exports))

# Plot the graph for 2000

ggplot(top_exports_2000_sum, aes(x = "", y = total_exports, fill = iso3_d)) + 
  geom_bar(width = 1, stat = "identity") +
  coord_polar(theta = "y") +
  labs(title = "Five main importers from Japan in 2000", fill = "Country", x = NULL, y = NULL) +
  theme_void() +
  theme(legend.position = "left")


# Top 5 export partners of Japan in 2012
...
# Plot the graph for 2012
ggplot(top_exports_2012_sum, aes(x = "", y = total_exports, fill = iso3_d)) + 
  geom_bar(width = 1, stat = "identity") +
  coord_polar(theta = "y") +
  labs(title = "Five main importers from Japan in 2012", fill = "Country", x = NULL, y = NULL) +
  theme_void() +
  theme(legend.position = "left")
```
...
## 2. Test (ACJEP's member states trade level)
...
(1) Put 2008 on the same line as the other years, below the plot.
(2) Review format of tables in Japan exports and imports to and from ASEAN countries.
(3) Graph with main trading partners.
(4) Trading partenrs Japan in the world: adding percentages.
