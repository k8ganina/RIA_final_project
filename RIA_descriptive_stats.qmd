...
as_hux(japan_imports) %>%
  ...
  set_right_border(everywhere, 1) %>%
  set_bottom_border(3, everywhere) %>%
  set_number_format(-1, -1, fmt_pretty(format=5))
```
...
ggplot(top_imports_2000_sum, aes(x = "", y = share_japan_imports, group=iso3_o)) + 
  geom_bar(width = 1, stat = "identity", fill=iso3_o) +
  coord_polar(theta = "y") +
  labs(title = "Five main exporters to Japan in 2000", fill = "Country", x = NULL, y = NULL) +
  theme_void() +
  theme(legend.position = "left") +
  geom_text(aes(label = paste0(round(share_japan_imports, 2), "%")), 
            position = position_stack(vjust = 0.5), size = 3.5) +
  scale_fill_discrete(labels = c("US", "Indonesia", "South Korea", "Taiwan", "China"))
```
